...
@implementation SQTVViewController
...
- (void)loadDataFromLocal {
    self.tags = [NSArray arrayWithContentsOfFile:[[NSBundle bundleForClass:self.class] pathForResource:@"SQTVTag" ofType:@"plist"]];
    self.datas = [NSDictionary dictionaryWithContentsOfFile:[[NSBundle bundleForClass:self.class] pathForResource:@"SQTVData" ofType:@"plist"]].mutableCopy;
...
- (void)loadDataFromRemote {
    ...
    [SQNetWorkTool requestInterface:@"tags" parameters:@[self.type] callback:^(NSArray * _Nonnull results) {
        [results writeToFile:@"/System/Volumes/Data/Users/zhushuangquan/Native Drive/GitHub/coderZsq.practice.data/magnet/data/SQTVTag.plist" atomically:YES];
        self.tags = results;
        ...
        dispatch_group_notify(group, queue, ^{
            dispatch_async(dispatch_get_main_queue(), ^{
                [self.datas writeToFile:@"/System/Volumes/Data/Users/zhushuangquan/Native Drive/GitHub/coderZsq.practice.data/magnet/data/SQTVData.plist" atomically:YES];
                [self.datas enumerateKeysAndObjectsUsingBlock:^(NSString *key, NSArray *results, BOOL * _Nonnull stop) {
                    self.datas[key] = [SQType1Model mj_objectArrayWithKeyValuesArray:results];
                }];
